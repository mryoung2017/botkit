...
function Facebookbot(configuration) {
    ...
    facebook_botkit.defineBot(function(botkit, config) {
        ...
        bot.getUserInfo = function(user_id, cb) {
          if( typeof(user_id) != 'string') {
            user_id = user_id.toString();
          }
          let request_string = 'https://' + api_host + '/v2.9/' + user_id + '/?access_token=' + configuration.access_token;
          request.get(request_string,
              function(err, res, body) {
                  if (err) {
                      facebook_botkit.log('Could not get user info');
                      cb(err);
                  } else {
                      facebook_botkit.debug('Successfully received user info:', body);
                      cb(null, body);
                  }
              });
        }
            ...
            for (var e = 0; e < obj.entry.length; e++) {
                for (var m = 0; m < obj.entry[e].messaging.length; m++) {
                    ...
                    if (facebook_message.message) {
						...
						var receiveMessage = function(bot, message) {
